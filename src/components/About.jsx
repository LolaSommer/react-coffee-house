...
<div className='about__img'>
  ...
  <img className="about__img" src={auracafe} loading="lazy" alt="A bright, cozy café space where coffee rituals come to pfe" />
  </picture>
</div>
</div>
...
<video  ref={vidRef} className="about__video"  muted playsInline aria-hidden="true">
 ...
 {!hasStarted && (<button type="button"  aria-label="Play background video" onClick={handleBigPlayVideo} className='video__btn'>Magic Process</button>)}
 {hasStarted &&(<button type="button" 
 aria-label={isPlay ? 'Pause video' : 'Play video'}
aria-pressed={isPlay}
  onClick={handleToggleplay}  className={`video__btn-small ${isPlay ? 'is-playing' : 'is-paused'}`}></button>)}
 </div>
